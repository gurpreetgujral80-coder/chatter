...
def run_selenium_script():
    """
    Initialize headless Chrome *after* the Flask server is reachable, then run the JS snippet.
    ...
    # If you don't use webdriver-manager, Selenium's manager may attempt to find/start driver.
    # If you need to point to a binary path, create Service('/path/to/chromedriver')
    driver = None
    try:
        ...
        while True:
            try:
                ...
                if time.time() - start > SELENIUM_STARTUP_TIMEOUT:
                    print("Selenium: server not reachable, aborting selenium startup.")
                    return
                time.sleep(SELENIUM_RETRY_INTERVAL)

        # create driver (may raise if binary missing)
        ...
        # keep driver alive as long as you want; if not needed, quit immediately:
        # driver.quit()
    except Exception as e:
        # log but don't crash the whole server
        import traceback
        print("Selenium startup error:", e)
        traceback.print_exc()
    finally:
        # optional: if you chose to quit the driver after work:
        # if driver:
        #     try: driver.quit()
        #     except: pass
        return

# Start the selenium thread *just before* starting socketio.run
def start_selenium_thread():
    t = threading.Thread(target=run_selenium_script, daemon=True)
    t.start()
