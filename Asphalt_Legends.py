...
/* call flow + minimal WebRTC signaling */
...
socket.on('call_declined', (data)=>{ alert('Call declined'); });
socket.on('call_accepted', async (data)=>{
  // start webRTC as caller
  currentCallId = data.call_id;
  await startLocalCall(true, data.call_id, data.from); // isCaller = true
});
...
socket.on('webrtc_offer', async (data)=>{
  const { sdp, from, call_id } = data;
  // if not already in call, prepare to answer
  if(pc) { console.warn('already have pc'); }
  await startLocalCall(false, call_id, from); // isCaller = false
  await pc.setRemoteDescription(new RTCSessionDescription(sdp));
  const answer = await pc.createAnswer();
  await pc.setLocalDescription(answer);
  socket.emit('webrtc_answer', { to: from, sdp: pc.localDescription, call_id });
});
...
byId('acceptCall')?.addEventListener('click', async ()=>{ if(!currentInvite) return; // accept -> notify server to set started and notify caller
  socket.emit('call_accept',{ call_id: currentInvite }); byId('incomingCall').style.display='none'; currentInvite=null;
});
...
// startLocalCall - sets up RTCPeerConnection and local stream
async function startLocalCall(isCaller, call_id, other){
  ...
  pc.ontrack = (ev)=>{ ev.streams[0].getTracks().forEach(t=> remoteStream.addTrack(t)); remoteAudio.srcObject = remoteStream; };

  pc.onicecandidate = (ev)=>{ if(ev.candidate){ socket.emit('ice_candidate', { to: other, candidate: ev.candidate }); } };

  try{
    const constraints = { audio:true, video: isCaller ? true : true }; // allow both
    ...
    if(isCaller){
      ...
      socket.emit('webrtc_offer', { to: other, sdp: pc.localDescription, from: myName, call_id });
    }
  }catch(e){ console.error('media error', e); alert('Could not access camera/mic: '+ e.message); }
}
...
# Asphalt_Legends.py (updated with WhatsApp-like call features)
import os
import sqlite3
import secrets
import time
